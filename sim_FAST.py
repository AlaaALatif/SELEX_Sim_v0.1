...
for r in range(roundNum):
    if(r==0):
        ...
        amplfdSeqs = Amplify.randomPCR_with_ErrorsAndBias_FAST(slctdSeqs, seqLength, pcrCycleNum, pcrYield, pcrErrorRate, aptamerSeqs, alphabetSet)
        ...
        for seqIdx in amplfdSeqs:
            nxtRnd.write(str(seqIdx)+'\t'+str(amplfdSeqs[seqIdx][0])+'\t'+str(amplfdSeqs[seqIdx][1])+'\t'+'\n') #write seqIdx, count, distance, and bias...for now
        nxtRnd.close()
    else:
        del(slctdSeqs)
